...
| Файлы приложений на сайте | `https://tunello.online/downloads/<файл>` | лежат в `/opt/tunnelo-site/downloads`, отдаёт nginx мимо python; кладёт их `publish-release.sh` (через Mac: между серверами SSH закрыт) |
...
**Вход: логин и пароль** (решено 16.09.2026). Подписка живёт на аккаунте, а не
на устройстве. Регистрация — по почте (адрес → код из письма), логин и пароль
выдаются при оплате и при первом входе по почте, письмом; промокод для своих
заменяет оплату и заканчивается тем же. Устройство привязывается к паре
логин + пароль. Пароли в таблице `users` только хешем (PBKDF2-SHA256).
Ручки: `/auth/login`, `/auth/forgot` (ссылка на смену, час, одноразовая),
`/auth/reset`, `/auth/password`, старые `/auth/request` и `/auth/verify` на
месте. В приложении вход обязателен: `TunneloAuthGate` стоит вокруг всей
оболочки с вкладками (`routing_config_notifier.dart`), поэтому без пары
логин-пароль не открывается ни главная, ни «Настройки» — на всех четырёх
платформах сразу, оболочку рисует общий виджет.

Тестовые аккаунты заводит `make-test-user.py` на сервере сервиса активации.
Он создаёт клиента в панели, но **не пишет строку в `activations`**, а
`/status` и `/me` ищут почту клиента именно там — без неё приложение считает
подписку неактивной. После скрипта дописывать строку
`(device, code, sub_id, email)` руками.

**Языки.** В приложении девять: русский, английский, азербайджанский,
грузинский, армянский, казахский, киргизский, узбекский, таджикский (плюс
доставшиеся от апстрима арабский, фарси и прочие). Файлы —
`assets/translations/*.i18n.json`, сверка с английским:
`python3 scripts/check_translations.py`. Новый язык в списке подписывается в
`lib/core/localization/locale_extensions.dart`, иначе покажется «Unknown»;
после правки файлов — `dart run slang`. На сайте те же девять: строки в
`site/i18n.py` (русский и английский) и `site/lang/<код>.py` (остальные),
переключатель в шапке, выбор запоминается печеньем `tunnelo_lang`.
Юридические документы остаются только на русском — это документы по
российскому праву.
...
**Иконка на Android живёт в двух местах.** `mipmap-*/ic_launcher.webp`
видят только системы до Android 8; с восьмой берётся
`mipmap-anydpi-v26/ic_launcher.xml` — adaptive-icon. Он оставался от апстрима
(синие столбики Hiddify), и в списке приложений стоял чужой значок, хотя
«иконка заменена». Правится вместе: и webp, и foreground с фоном
adaptive-icon (22.09.2026).

**На Android нет системного id устройства.** `androidInfo.id` из
device_info_plus — это Build.ID, метка прошивки, одинаковая у всех
телефонов с одной версией системы; до 1.0.29 сервер по нему выдавал один
ключ разным людям. Id на Android — только свой случайный из настроек.
...
- Перед коммитом: `flutter analyze` должен быть чистым. Правил сайт — ещё
  `/opt/homebrew/bin/python3.11 site/test_site.py` (ловит потерю /internal/mail
  и расхождение языков). Правил сервис активации —
  `/opt/homebrew/bin/python3.11 server/test_activation.py`. Правил переводы —
  `/opt/homebrew/bin/python3.11 scripts/check_translations.py`.
