...
func BuildConfig(ctx context.Context, hopts *HiddifyOptions, inputOpt *ReadOptions) (*option.Options, error) {
	...
	// Tunnelo: dns/route строим сами (см. tunnelo_dns.go) — конфиг
	// проверен трафиком на sing-box 1.13, стандартные setDns и
	// setRoutingOptions не вызываются.
	if err := setTunneloDns(&options); err != nil {
		return nil, err
	}
	setTunneloRoute(&options, hopts)
