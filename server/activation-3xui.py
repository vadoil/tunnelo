...
Эндпоинты (для приложения):
  POST /activate      {"code":"КОД","device":"<hwid>"}
...
# Бесплатного месяца больше нет (16.09.2026): подписка появляется только после
# оплаты или именного промокода. Общий код PARDAUTO из посева убран, а в базе
# ему выставлен лимит по числу уже выданных активаций — старые ключи живут,
# новых по нему не выдаётся.
PROMO_CODES = {
    # Именные коды: лимит 1, чтобы код нельзя было передать дальше, и чтобы
    # в базе было видно, чья это активация.
